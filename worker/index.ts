...
async function issueGeminiToken(env: Env) {
  if (!env.GEMINI_API_KEY) return json({ error: 'GEMINI_API_KEY is not configured' }, { status: 500 });
  ...
  if (!response.ok) {
    ...
    return json({ error: 'Could not create a Gemini ephemeral token', status: response.status }, { status: 502 });
  }

  const token = (await response.json()) as { name?: string };
  if (!token.name) return json({ error: 'Gemini returned an invalid token response' }, { status: 502 });
