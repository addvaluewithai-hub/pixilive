...
(function(host){
 ...
 function createRig(root,options={}){
  ...
  if(options.preset==='octo')return host.OctopusMotion.createRig(root,options);
  ...
  const q={...faces.neutral,...rest,...G.mouthBase,mouthSmile:.65,lift:0,wave:0,voice:0};
  ...
  function targets(){
   ...
   Object.assign(goal,rest,poses[active]||{}, {lift:0,wave:active==='wave'&&!reduced?1:0,voice:0});goal.lean+=faceLean*.55;
  ...
  function draw(){
   ...
   attr('m-shadow','transform',`translate(303 488) scale(${f(clamp(1+q.lift/180,.55,1.1))} 1) translate(-303 -488)`);
   ...
   for(const side of ['l','r']){
    const rx=side==='l'?23:31,ry=side==='l'?27:35,open=clamp(q[side==='l'?'openL':'openR']*(1-blink),.005,1.25),top=-ry*1.333*open,bottom=ry*1.333*open;
   ...
   attr('mouth','transform',`translate(${f(303+q.mouthOffset*.7)} 324) scale(.95 1.15)`);attr('mouth-edge','stroke-width',4);
  }
