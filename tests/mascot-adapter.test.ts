...
for(const mascotId of ['fustuq','octo'])test(mascotId+' adapter preserves the existing speech/perform contract and uses its own renderer',()=>{
 const events:unknown[][]=[],definition=getCharacter(mascotId);
 ...
 try{
  ...
  assert.ok(portrait(engine,definition).startsWith('data:image/svg+xml'));assert.ok(performanceInstructions({name:definition.name,species:definition.species}).includes(mascotId==='octo'?'Octo/أوكتو':'Fustuq/فستق'));
